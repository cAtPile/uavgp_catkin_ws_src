...
{
    // 初始化订阅器
    state_sub = nh.subscribe<mavros_msgs::State>("/mavros/state", 10, &MissionMaster::stateCheckCB, this);
...
{
    double dx = current_pose.pose.position.x - pose_v3d.x();
    double dy = current_pose.pose.position.y - pose_v3d.y();
    double dz = current_pose.pose.position.z - pose_v3d.z();
