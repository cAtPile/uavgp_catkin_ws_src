...
void AvoidPlanner::generateForceDir() {
    ...
    const double el_res = current_polar_field_.elevation_resolution;  // 俯仰角分辨率
    const double min_az = current_polar_field_.min_azimuth;          // 最小方位角
    const double min_el = current_polar_field_.min_elevation;        // 最小俯仰角
    ...
    for (size_t az_idx = 0; az_idx < num_az_bins; ++az_idx) {
        for (size_t el_idx = 0; el_idx < num_el_bins; ++el_idx) {
            ...
            ROS_INFO("force[%d][%d]=(%0.2f,%0.2f,%0.2f)",az_idx,el_idx,x_force,y_force,z_force);
            // 4. 累加至总体合力矢量
