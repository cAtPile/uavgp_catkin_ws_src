...
apoc::apoc(): rate(20.0){
     ...
     nh_.param("connect_timeout", connect_timeout_, 10.0);
     ROS_INFO("connect_timeout: %.1fs", connect_timeout_);
     nh_.param("modeswitch_timeout", modeswitch_timeout_, 10.0);
     ROS_INFO("modeswitch_timeout: %.1fs", modeswitch_timeout_);
     nh_.param("armswitch_timeout", armswitch_timeout_, 10.0);
     ROS_INFO("armswitch_timeout: %.1fs", armswitch_timeout_);
     nh_.param("fly_ab_timeout", fly_ab_timeout_, 30.0);
     ROS_INFO("fly_ab_timeout: %.1fs", fly_ab_timeout_);
     nh_.param("reach_tolerance_distance", reach_tolerance_distance_, 0.1);
     ROS_INFO("reach_tolerance_distance: %.1fm", reach_tolerance_distance_);
     nh_.param("reach_tolerance_angle", reach_tolerance_angle_, 0.1);
     ROS_INFO("reach_tolerance_angle: %.1frad", reach_tolerance_angle_);
     
         // ============== PID参数获取（新增） ==============
    // X轴PID
    nh_.param("apoc_pkg/PID_X_KP", pid_x_kp_, 2.5);
    ROS_INFO("PID_X_KP: %.2f", pid_x_kp_);
    nh_.param("apoc_pkg/PID_X_KI", pid_x_ki_, 0.1);
    ROS_INFO("PID_X_KI: %.2f", pid_x_ki_);
    nh_.param("apoc_pkg/PID_X_KD", pid_x_kd_, 0.05);
    ROS_INFO("PID_X_KD: %.2f", pid_x_kd_);
    nh_.param("apoc_pkg/PID_X_OUT_MIN", pid_x_out_min_, -1.0);
    ROS_INFO("PID_X_OUT_MIN: %.2f", pid_x_out_min_);
    nh_.param("apoc_pkg/PID_X_OUT_MAX", pid_x_out_max_, 1.0);
    ROS_INFO("PID_X_OUT_MAX: %.2f", pid_x_out_max_);
    nh_.param("apoc_pkg/PID_X_INT_MIN", pid_x_int_min_, -0.5);
    ROS_INFO("PID_X_INT_MIN: %.2f", pid_x_int_min_);
    nh_.param("apoc_pkg/PID_X_INT_MAX", pid_x_int_max_, 0.5);
    ROS_INFO("PID_X_INT_MAX: %.2f", pid_x_int_max_);

    // Y轴PID
    nh_.param("apoc_pkg/PID_Y_KP", pid_y_kp_, 2.5);
    ROS_INFO("PID_Y_KP: %.2f", pid_y_kp_);
    nh_.param("apoc_pkg/PID_Y_KI", pid_y_ki_, 0.1);
    ROS_INFO("PID_Y_KI: %.2f", pid_y_ki_);
    nh_.param("apoc_pkg/PID_Y_KD", pid_y_kd_, 0.05);
    ROS_INFO("PID_Y_KD: %.2f", pid_y_kd_);
    nh_.param("apoc_pkg/PID_Y_OUT_MIN", pid_y_out_min_, -1.0);
    ROS_INFO("PID_Y_OUT_MIN: %.2f", pid_y_out_min_);
    nh_.param("apoc_pkg/PID_Y_OUT_MAX", pid_y_out_max_, 1.0);
    ROS_INFO("PID_Y_OUT_MAX: %.2f", pid_y_out_max_);
    nh_.param("apoc_pkg/PID_Y_INT_MIN", pid_y_int_min_, -0.5);
    ROS_INFO("PID_Y_INT_MIN: %.2f", pid_y_int_min_);
    nh_.param("apoc_pkg/PID_Y_INT_MAX", pid_y_int_max_, 0.5);
    ROS_INFO("PID_Y_INT_MAX: %.2f", pid_y_int_max_);

    // Z轴PID
    nh_.param("apoc_pkg/PID_Z_KP", pid_z_kp_, 3.0);
    ROS_INFO("PID_Z_KP: %.2f", pid_z_kp_);
    nh_.param("apoc_pkg/PID_Z_KI", pid_z_ki_, 0.2);
    ROS_INFO("PID_Z_KI: %.2f", pid_z_ki_);
    nh_.param("apoc_pkg/PID_Z_KD", pid_z_kd_, 0.1);
    ROS_INFO("PID_Z_KD: %.2f", pid_z_kd_);
    nh_.param("apoc_pkg/PID_Z_OUT_MIN", pid_z_out_min_, -0.8);
    ROS_INFO("PID_Z_OUT_MIN: %.2f", pid_z_out_min_);
    nh_.param("apoc_pkg/PID_Z_OUT_MAX", pid_z_out_max_, 0.8);
    ROS_INFO("PID_Z_OUT_MAX: %.2f", pid_z_out_max_);
    nh_.param("apoc_pkg/PID_Z_INT_MIN", pid_z_int_min_, -0.4);
    ROS_INFO("PID_Z_INT_MIN: %.2f", pid_z_int_min_);
    nh_.param("apoc_pkg/PID_Z_INT_MAX", pid_z_int_max_, 0.4);
    ROS_INFO("PID_Z_INT_MAX: %.2f", pid_z_int_max_);

    // Yaw角PID
    nh_.param("apoc_pkg/PID_YAW_KP", pid_yaw_kp_, 2.0);
    ROS_INFO("PID_YAW_KP: %.2f", pid_yaw_kp_);
    nh_.param("apoc_pkg/PID_YAW_KI", pid_yaw_ki_, 0.05);
    ROS_INFO("PID_YAW_KI: %.2f", pid_yaw_ki_);
    nh_.param("apoc_pkg/PID_YAW_KD", pid_yaw_kd_, 0.02);
    ROS_INFO("PID_YAW_KD: %.2f", pid_yaw_kd_);
    nh_.param("apoc_pkg/PID_YAW_OUT_MIN", pid_yaw_out_min_, -0.6);
    ROS_INFO("PID_YAW_OUT_MIN: %.2f", pid_yaw_out_min_);
    nh_.param("apoc_pkg/PID_YAW_OUT_MAX", pid_yaw_out_max_, 0.6);
    ROS_INFO("PID_YAW_OUT_MAX: %.2f", pid_yaw_out_max_);
    nh_.param("apoc_pkg/PID_YAW_INT_MIN", pid_yaw_int_min_, -0.3);
    ROS_INFO("PID_YAW_INT_MIN: %.2f", pid_yaw_int_min_);
    nh_.param("apoc_pkg/PID_YAW_INT_MAX", pid_yaw_int_max_, 0.3);
